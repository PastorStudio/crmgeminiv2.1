...
app.get('/auto-response/config', async (req, res) => {
  try {
    const result = await pool.query('SELECT * FROM auto_response_config WHERE id = 1');
    ...
    if (result.rows.length === 0) {
      ...
      const defaultConfig = await pool.query(
        ...
        VALUES (1, false, 'Gracias por contactarnos. En breve un asesor le atenderá.', 
...
app.post('/auto-response/config', async (req, res) => {
  try {
    ...
    // Construir consulta dinámica
    let query = 'UPDATE auto_response_config SET ';
    const params = [];
    const updates = [];
    ...
    if (greetingMessage) {
      ...
      updates.push(`"greetingMessage" = $${params.length}`);
    }
    
    if (outOfHoursMessage) {
      params.push(outOfHoursMessage);
      updates.push(`"outOfHoursMessage" = $${params.length}`);
    }
    
    if (businessHoursStart) {
      params.push(businessHoursStart);
      updates.push(`"businessHoursStart" = $${params.length}`);
    }
    
    if (businessHoursEnd) {
      params.push(businessHoursEnd);
      updates.push(`"businessHoursEnd" = $${params.length}`);
    }
    
    if (workingDays) {
      params.push(workingDays);
      updates.push(`"workingDays" = $${params.length}`);
    }
    
    if (geminiApiKey) {
      params.push(geminiApiKey);
      updates.push(`"geminiApiKey" = $${params.length}`);
    }
    ...
    if (result.rows.length === 0) {
      // Si no existe configuración, crear una por defecto con los valores proporcionados
      const insertParams = [
      ...
      const insertResult = await pool.query(
        `INSERT INTO auto_response_config 
        (id, enabled, "greetingMessage", "outOfHoursMessage", "businessHoursStart", "businessHoursEnd", "workingDays", "geminiApiKey", settings) 
