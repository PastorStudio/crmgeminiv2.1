...
// Limpiar completamente una sesión de WhatsApp (eliminar archivos y conexión)
router.post('/:id/clear-session', async (req, res) => {
  try {
    ...
    // Limpiar sesión
    const success = await whatsappMultiAccountManager.clearWhatsAppSession(id);
    if (!success) {
      return res.status(500).json({ error: 'Error al limpiar sesión de WhatsApp' });
    }
    
    res.json({ 
      success: true, 
      message: `Sesión para cuenta "${account.name}" limpiada completamente` 
    });
  } catch (error) {
    console.error('Error al limpiar sesión de WhatsApp:', error);
    res.status(500).json({ error: 'Error al limpiar sesión de WhatsApp' });
  }
});

// Limpiar todas las sesiones de WhatsApp
router.post('/clear-all-sessions', async (req, res) => {
  try {
    // Limpiar todas las sesiones
    const success = await whatsappMultiAccountManager.cleanAllSessions();
    if (!success) {
      return res.status(500).json({ error: 'Error al limpiar todas las sesiones de WhatsApp' });
    }
    
    res.json({ 
      success: true, 
      message: 'Todas las sesiones de WhatsApp han sido limpiadas correctamente' 
    });
  } catch (error) {
    console.error('Error al limpiar todas las sesiones de WhatsApp:', error);
    res.status(500).json({ error: 'Error al limpiar todas las sesiones de WhatsApp' });
  }
});
